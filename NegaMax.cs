...
namespace ChessEnginePort
{
    ...
    static class NegaMax
    {
        ...
        public static bool nullMove = false;
        ...
        public static Evaluation iterativeDeepeningMTDF(Crawler crawler, ChessBoard.PieceColor color, List<ChessBoard.Move> moves, int depth, int time)
        {
            ...
            for (int i = 2; i <= depth; i++)
            {
                Evaluation testEval = mtdf(crawler, color, moves, i, bestEval.score);
            ...
            transpositions = new Dictionary<ulong, Transposition>();
            killerMoves.Clear();
        ...
        public static Evaluation mtdf(Crawler crawler, ChessBoard.PieceColor color, List<ChessBoard.Move> moves, int depth, int guess)
        {
            ...
            while (upperBound > lowerBound)
            {
                ...
                g = alphaBetaWithMemory(crawler, color, moves, depth, 0, beta - 1, beta, false);
        ...
        public static Evaluation alphaBetaWithMemory(Crawler crawler, ChessBoard.PieceColor color, List<ChessBoard.Move> moves, int depth, int ply, int alpha, int beta, bool allowNull)
        {
            //Time
            if (sw.ElapsedMilliseconds > timeCut)
            ...
            if (crawler.isDraw())
            {   
                return new Evaluation(DRAW_SCORE, moves.ToArray());
            }
            // Transpositions
            ...
            if (nullMove && allowNull && depth > 3)
            {
                ...
                if (!ChessBoard.kingInDanger(crawler.currentPosition, kingColor))
                {
                    Evaluation nullScore;
                    crawler.flipPosition();

                    if (negate == 1)
                    {
                        nullScore = alphaBetaWithMemory(crawler, color, moves, depth - 3, ply + 2, beta - 1, beta, false);
                    ...
                    {
                        nullScore = alphaBetaWithMemory(crawler, color, moves, depth - 3, ply + 2, alpha, alpha + 1, false);

                        crawler.flipPosition();

                        if (nullScore.score < alpha)
                            return nullScore;
                    }
                }
            }
            ...
            foreach (ChessBoard.Move move in nextMoves)
            {
                crawler.move(move);
                moves.Add(move);
                Evaluation moveScore = alphaBetaWithMemory(crawler, color, moves, depth - 1, ply + 1, alpha, beta, !allowNull);
                crawler.undoMove();
